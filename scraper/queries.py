...
def return_companies():
    return [company.name for company in session.query(Company).all() ]
# print(return_companies())

def return_industries():
    return [industry.name for industry in session.query(Industry).all() ]
# print(return_industries())


def top_10_industries_witht_highest_job_listings():
    listy =session.query(func.count(Job.id), Industry.name).join(Industry).order_by(func.count(Job.id).desc()).group_by(Industry.name).all()
    top_10 = []
    for item in listy:
        if item[1]==None:
            listy.remove(item)
